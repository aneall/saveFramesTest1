...
public class ReadPoseUpdateCameraFinal : MonoBehaviour
{
    // SCRIPT V5 – successfully reads pose from Telepresence OR Office CSV file, then updates camera (translation AND rotation)
        // assumes sampling rate (from Aria glasses data) is 1000 samples per second (aka 1 timestamp sample per millisecond)
        // TODO: clean up code, make comments better
    ...
    void Update()
    {
        ...
        // THIS WORKS!!!
        Vector3 ariaForward = relAriaRot * Vector3.forward;
        //Vector3 ariaUp = relAriaRot * Vector3.up;

        Vector3 unityForward = new Vector3(-ariaForward.y, -ariaForward.x, ariaForward.z);
        //Vector3 unityUp = new Vector3(ariaUp.y, -ariaUp.x, ariaUp.z);

        Quaternion unityRot = Quaternion.LookRotation(unityForward, Vector3.up);
        ...
        // Console output for rotation debugging rotation, commenting out for better readability w timing:
    ...
    void ReadCSV()
    {
        ...
        for (int i = 0; i < tableSize; i++)
        {
            ...
            poseTimestampsSec.Add((i - 1) * 0.001f);
